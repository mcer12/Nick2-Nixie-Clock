...
static const char html_header[] PROGMEM =
  ...
  "<title>NICK2 - Wi-Fi Nixie Clock</title>"
  "<style>"
...
#ifdef CLOCK_6_DIGIT
        if (i < 4 && splitTime[i] == 0 && digitsCount > 4) {
          //blankDigit(i);
          continue;
        }
#endif
...
void handleRoot() {
  ...
  if (!server.args() || (server.args() && server.hasArg("is_form"))) {
    ...
    html += "<h1>NICK2 - Wi-Fi Nixie Clock</h1> <p></p>";
    ...
    */
    ...
    html += "<option value=\"1\"";
    if (json["zero"].isNull() || json["t_format"].as<int>() == 1) html += " selected";
    html += ">24-hour</option>";
    html += "<option value=\"0\"";
    ...
    html += "<option value=\"1\"";
    if (json["zero"].isNull() || json["zero"].as<int>() == 1) html += " selected";
    html += ">Show</option>";
    html += "<option value=\"0\"";
    ...
    html += "<option value=\"0\"";
    if (json["zero"].isNull() || json["rst_cycle"].as<int>() == 0) html += " selected";
    html += ">No</option>";
    html += "<option value=\"1\"";
    ...
    html += "<option value=\"1\"";
    if (json["rst_ip"].isNull() || rst_ip == 1) html += " selected";
    html += ">Yes</option>";
    html += "<option value=\"0\"";
    ...
    html += "<div class=\"row\"><label for=\"fade\">Crossfade duration:</label>";
    html += "<select id=\"fade\" name=\"fade\">";
    html += "<option value=\"600\"";
    if (json["fade"].as<int>() == 600) html += " selected";
    html += ">Slow (600ms)</option>";
    html += "<option value=\"400\"";
    if (json["fade"].isNull() || json["fade"].as<int>() == 400) html += " selected";
    html += ">Medium (400ms)</option>";
    html += "<option value=\"200\"";
    if (json["fade"].as<int>() == 200) html += " selected";
    html += ">Fast (200ms)</option>";
    html += "<option value=\"0\"";
