...
static const char html_header[] PROGMEM =
  ...
  "<title>NICK2 - Wi-Fi Nixie Clock (IN-12)</title>"
  "<style>"
...
boolean captivePortal() {
  if (deviceMode == CONFIG_MODE && !isIp(server.hostHeader()) && server.hostHeader() != String(dns_name) + ".local") {
    ...
    handleRoot();
    //server.sendHeader("Location", toStringIp(server.client().localIP()), true);
...
#if !defined(CLOCK_VERSION_IN12) && !defined(CLOCK_VERSION_IV22)
